...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document
    .getElementById('close-tabs-all')
    .addEventListener('click', () => chrome.runtime.sendMessage({ msg: 'closeAllTabs' }));

  document
    .getElementById('close-tabs-left')
    .addEventListener('click', () => chrome.runtime.sendMessage({ msg: 'closeTabsLeft' }));

  document
    .getElementById('close-tabs-right')
    .addEventListener('click', () => chrome.runtime.sendMessage({ msg: 'closeTabsRight' }));

  document
    .getElementById('show-tabs')
    .addEventListener('click', () =>
      chrome.runtime.sendMessage({ msg: 'showTabs' }, (response) => toggleListForOpenTabs(response.currentTabs))
    );

  document
    .getElementById('btn-open-tabs-select-all')
    .addEventListener('click', () => checkAllBoxes(Array.from(document.querySelectorAll('#tabs-list>li'))));

  document
    .getElementById('btn-open-tabs')
    .addEventListener('click', () => closeSelectedTabs(Array.from(document.querySelectorAll('#tabs-list>li'))));
});

const checkAllBoxes = (list) => {
  list.forEach((element) => (element.firstChild.checked = true));
};
...
const toggleListForOpenTabs = (currentTabs = null) => {
  ...
  if (!listOpen) {
    ...
    currentTabs.forEach((tab) => {
      ...
      li.innerHTML = `<input class="checkbox-open-tabs" type="checkbox" name="check-${tab.id}" id="check-${tab.id}" /> <a class="ul-open-tabs" id="link-${tab.id}">${tab.title}</a>`;
